...
class CodWaiting extends react__WEBPACK_IMPORTED_MODULE_8__["Component"] {
  constructor() {
    super();
      ...
      antd_lib_message__WEBPACK_IMPORTED_MODULE_5___default.a.loading('upload...', 2).then(() => {
        if (excel.length > 0) {
          new _babel_runtime_corejs2_core_js_promise__WEBPACK_IMPORTED_MODULE_4___default.a((resolve, rejects) => {
            var a = Object(_apis_datas__WEBPACK_IMPORTED_MODULE_10__["Upfile"])(user, transport_comp, excel, store);
            resolve(a);
          }).then(res => {
            let code = res.code,
                mess = res.message;
